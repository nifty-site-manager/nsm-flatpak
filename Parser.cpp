                         ...
                         const std::string& WinTextEditor)
{
    ...
    symbol_table.add_function("rec", r);
...
int Parser::interactive(std::string& lang, std::ostream& eos)
{
    ...
    while(1)
    {
        ...
        result = getline(lang, addPath, '$', inLine, 1);
        if(result == NSM_KILL)
            return NSM_KILL;

        while(result == NSM_SENTER || (inLine.size() && inLine[inLine.size()-1] == '\\'))
        {
            if(result == NSM_SENTER)
                toProcess += inLine;
            else
        ...
        else if(lang[0] == 'e')
        {
            if(!exprset.add_expr(toProcess))
            {
                ...
                if(!consoleLocked)
                    os_mtx->lock();
                start_err(eos, emptyPath, 1) << c_green << "exprtk" << c_white << ": failed to compile expression" << std::endl;
...
int Parser::build(const TrackedInfo& ToBuild, std::ostream& eos)
{
    mode = MODE_BUILD;
                                ...
                                std::ostream& eos)
{
    ...
    if(linePos < inStr.size())
    {
        if(inStr[linePos] == '`') //expression
        {
            ...
            if(parseExpr)
            {
                ...
                value = vars.double_to_string(expr.evaluate(), 1);
            }
            else
            {
                ...
                value = vars.double_to_string(exprset.evaluate(expr_str), 1);
            }
            ...
            else if(inStr[linePos] == '\\')
            {
                ...
                switch(inStr[linePos])
                {
                    ...
                    case 'n': //this isn't needed but here in case code is moved before @\\n code
                        ++linePos;
                        indentAmount = baseIndentAmount;
                        /*if(codeBlockDepth) //baseIndentAmount updated at start of code block
                            outStr += "\n";
                        else*/
                        outStr += "\n" + baseIndentAmount;
                        break;
        ...
        else if(inStr[linePos] == '!')
        {
            if(inStr.substr(linePos, 3) == "!\\n")
            {
                ...
                if(restOfLine.find("@!\\n") != std::string::npos)
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    start_err(eos, readPath, lineNo) << "do not use @!\\n twice on the same line" << std::endl;
                    os_mtx->unlock();
                    return 1;
                }
                ...
                if(n_read_and_process_fast(0, 0, restOfLine, lineNo-1, readPath, antiDepsOfReadPath, commentOutput, eos))
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    start_err(eos, readPath, lineNo) << "failed to parse @!\\n comment" << std::endl;
                    os_mtx->unlock();
                    return 1;
                }
        ...
        {
            if(inStr.substr(linePos, 3) == "---" && inStr[linePos + 3] != '{' && inStr[linePos + 3] != '(')
            {
                linePos += 3;
                int openLine = lineNo;
                ...
                else
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    start_err(eos, readPath, openLine) << "open/close comment --- has no close/open ---" << std::endl;
                    os_mtx->unlock();
                    return 1;
                }
            ...
            else if(inStr.substr(linePos, 3) == "-->" && inStr[linePos + 3] != '{' && inStr[linePos + 3] != '(')
            {
                linePos += 2;
    ...
    if(funcName[0] == '$')
    {
        if(funcName == "$")
        {
            ...
            else if(vars.find(varName, vpos)) //should this go after hard-coded constants? should we parse the string when printing?
            {
                if(!vars.add_str_from_var(vpos, outStr, 1, indent, indentAmount))
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    start_err_ml(eos, readPath, sLineNo, lineNo) << "$" << brackets << ": can't get string of var type " << quote(vpos.type) << std::endl;
                    os_mtx->unlock();
                    return 1;
                }
            }
    ...
    else if(funcName[0] == 'i')
    {
        ...
        else if(funcName == "if")
        {
            ...
            bool addScope = 0, addOut = addOutput;
            ...
            for(size_t b=0; b<blocks.size(); b++)
            {
                if(b < conditions.size())
                {
                    ...
                    if(!get_bool(result, parsedCondition, readPath, conditionLineNo, "if/else-if()", eos))
                        return 1;
                }
                else
                    result = 1;
        ...
        else if(funcName == "is-const")
        {
            if(params.size() != 1)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "is-const(): expected 1 parameter, got " << params.size() << std::endl;
                os_mtx->unlock();
                return 1;
            }
            ...
            else
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "is-const(): no variables or functions named " << c_light_blue << params[0] << c_white << std::endl;
                os_mtx->unlock();
                return 1;
            }
        ...
        else if(funcName == "is-private")
        {
            if(params.size() != 1)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "is-private(): expected 1 parameter, got " << params.size() << std::endl;
                os_mtx->unlock();
                return 1;
            }
            ...
            else
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "is-private(): no variables or functions named " << c_light_blue << params[0] << c_white << std::endl;
                os_mtx->unlock();
                return 1;
            }
        ...
        else if(funcName == "imginclude")
        {
            ...
            if(!file_exists(imgPathStr.c_str()))
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_warn(eos, readPath, lineNo) << "imginclude(): img file " << imgPathStr << " does not exist" << std::endl;
                if(!consoleLocked)
                    os_mtx->unlock();
            }
        ...
        else if(funcName == "in")
        {
            ...
            else
            {
                ...
                if(result)
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    start_err_ml(eos, readPath, sLineNo, lineNo) << "in{file}(): text editor system call failed" << std::endl;
                    os_mtx->unlock();
                    return 1;
                }
                ...
                if(!file_exists(output_filename))
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    start_err_ml(eos, readPath, sLineNo, lineNo) << "in{file}(): user did not save file" << std::endl;
                    os_mtx->unlock();
                    return 1;
                }
                ...
                if(antiDepsOfReadPath.count(inputPath))
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    start_err_ml(eos, readPath, sLineNo, lineNo) << "in{file}(): inputting file " << inputPath << " would result in an input loop" << std::endl;
                    os_mtx->unlock();
                    return 1;
                }
                ...
                if(n_read_and_process_fast(1, fileStr, 0, inputPath, antiDepsOfReadPath, outStr, eos) > 0)
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    start_err_ml(eos, readPath, sLineNo, lineNo) << "in{file}(): failed to parse input user saved to file" << std::endl;
                    os_mtx->unlock();
                    return 1;
                }
        ...
        else if(funcName == "ls")
        {
            ...
            if(!hasUnknownOptions && params.size() == 1 && (pos == std::string::npos || pos == params[0].size()-1))
            {
                ...
                if(toConsole)
                {
                    if(!consoleLocked)
                        os_mtx->lock();
                    coutPaths(lsPath.dir, paths, separator);
                    std::cout << std::endl;
                    if(!consoleLocked)
                        os_mtx->unlock();
                }
    ...
    else if(funcName[0] == 'p')
    {
        ...
        else if(funcName == "parse")
        {
            if(params.size() != 1 && params.size() != 2)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "parse(): expected 1-2 parameters, got " << params.size() << std::endl;
                os_mtx->unlock();
                return 1;
            }
            ...
            if(params.size() == 1)
            {
                if(parse_replace(lang, params[0], "parse string", readPath, antiDepsOfReadPath, lineNo, "parse()", sLineNo, eos))
                    return 1;
    ...
    else if(funcName[0] == 'c')
    {
        ...
        else if(funcName == "cssinclude")
        {
            ...
            if(!file_exists(cssPathStr.c_str()))
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_warn(eos, readPath, lineNo) << "cssinclude(): css file " << cssPathStr << " does not exist" << std::endl;
                if(!consoleLocked)
                    os_mtx->unlock();
            }
    ...
    else if(funcName[0] == '!')
    {
        if(funcName == "!")
        {
            ...
            if(isDouble(params[0]))
            {
                if(isInt(params[0]))
                {
                    if(std::atoi(params[0].c_str()))
                        result = "0";
                }
                else if(std::strtod(params[0].c_str(), NULL))
                    result = "0";
            }
            else if(params[0] != "")
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "@!(): cannot convert " << quote(params[0]) << " to bool" << std::endl;
                os_mtx->unlock();
                return 1;
            }*/
    ...
    else if(funcName[0] == ':')
    {
        if(funcName == ":=")
        {
            ...
            else if(valid_type(varType, readPath, antiDepsOfReadPath, lineNo, "valid_type(" + varType + ")", sLineNo, eos))
            {
                    ...
                    else
                    {
                        ...
                        else */if(vars.typeDefs.count(preType))
                        {
                            ...
                            if(vars.layers[layer].typeOf.count("options.size"))
                            {
                                ...
                                if(oldOptTypes[options.size()] == "int")
                            ...
                            if(vars.layers[layer].typeOf.count("params.size"))
                            {
                                ...
                                if(oldParTypes[params.size()] == "int")
                            ...
                            vars.layers[layer].typeOf["types.size"] = "int";
                            ...
                            vars.layers[layer].typeOf["params.size"] = "int";
                            ...
                            vars.layers[layer].typeOf["options.size"] = "int";
    ...
    else if(funcName[0] == 'w')
    {
        ...
        else if(funcName == "while")
        {
            ...
            {
                if(params[0].size() > 1 && params[0][0] == '`' && params[0][params[0].size()-1] == '`')
                {
                    exprtkCond = 1;
                    params[0] = params[0].substr(1, params[0].size()-2);
                }
            ...
            while(1)
            {
                ...
                else
                {
                    ...
                    if(!get_bool(result, parsedCondition, readPath, conditionLineNo, "while()", eos))
                    {
                        if(!consoleLocked)
                            os_mtx->lock();
    ...
    else if(funcName[0] == 'f')
    {
        ...
        else if(funcName == "for")
        {
            ...
            else if(exprtkPre)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                print_exprtk_parser_errs(eos, preExpr.parser, preExpr.expr_str, readPath, sLineNo);
                os_mtx->unlock();
                return 1;
            }
            else
            ...
            {
                if(params[1].size() > 1 && params[1][0] == '`' && params[1][params[1].size()-1] == '`')
                {
                    exprtkCond = 1;
                    params[1] = params[1].substr(1, params[1].size()-2);
                }
                condExpr.register_symbol_table(symbol_table);
            ...
            if(params[2].size() > 1 && params[2][0] == '`' && params[2][params[2].size()-1] == '`')
            {
                exprtkInc = 1;
                params[2] = params[2].substr(1, params[2].size()-2);
            }
            if(!exprtkInc)
            {
                ...
                if(!hasIncrements)
                {
                    ...
                    else if(exprtkInc)
                    {
                        if(!consoleLocked)
                            os_mtx->lock();
                        print_exprtk_parser_errs(eos, incExpr.parser, incExpr.expr_str, readPath, sLineNo);
                        os_mtx->unlock();
                        return 1;
                    }
                }
        ...
        else if(funcName == "faviconinclude")
        {
            ...
            if(!file_exists(faviconPathStr.c_str()))
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_warn(eos, readPath, lineNo) << "faviconinclude(): favicon file " << faviconPathStr << " does not exist" << std::endl;
                if(!consoleLocked)
                    os_mtx->unlock();
            }
    ...
    else if(funcName[0] == 'e')
    {
        if(funcName == "exprtk")
        {
            ...
            else
            {
                ...
                value = vars.double_to_string(exprset.evaluate(params[0]), 1);
            }
    ...
    {
        if(funcName == "out") //do we want this?
        {
            if(params.size() != 1)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "out(): expected 1 parameter, got " << params.size() << std::endl;
                os_mtx->unlock();
                return 1;
            }
    ...
    else if(funcName[0] == 'd')
    {
        if(funcName == "dep")
        {
            if(params.size() != 1)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "dep(): expected 1 parameter, got " << params.size() << std::endl;
                os_mtx->unlock();
                return 1;
            }
        ...
        else if(funcName == "do-while")
        {
            ...
            while(1)
            {
                ...
                else
                {
                    ...
                    if(!get_bool(result, parsedCondition, readPath, conditionLineNo, "do-while()", eos))
                    {
                        if(!consoleLocked)
                            os_mtx->lock();
                        start_err(eos, readPath, sLineNo) << funcName << "(): possible errors from exprtk:" << std::endl;
        ...
        else if(funcName == "dir")
        {
            if(params.size() != 0 && params.size() != 1)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "dir(): expected 0-1 parameters, got " << params.size() << std::endl;
                os_mtx->unlock();
                return 1;
            }
            ...
            bool toConsole = 0;
            std::string sys_call;
            std::string output_filename = ".@diroutput" + std::to_string(sys_counter++);
            ...
            #if defined _WIN32 || defined _WIN64
                sys_call = "dir " + params[0];
            #else
                sys_call = "ls " + params[0];
            #endif // defined

            int result;
            if(file_exists("/.flatpak-info")) //sys_call has to be quoted for cURL to work
            ...
            if(result)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "dir(" << quote(params[0]) << ") failed, ";
                eos << "see " << quote(output_filename) << " for system output" << std::endl;
                os_mtx->unlock();
                //remove_file(Path("./", output_filename));
                return 1;
            }
            ...
            if(toConsole)
            {
                ...
                {
                    if(fileLine.find_first_of('.') == std::string::npos)
                        eos << c_light_blue << fileLine << c_white << std::endl;
                    else
                        eos << fileLine << std::endl;
                }
                if(!consoleLocked)
                    os_mtx->unlock();
            }
    ...
    else if(funcName[0] == 's')
    {
        if(funcName == "size")
        {
            ...
            if(replaceVars)
            {
                ...
                if(vars.find(params[0], vpos))
                {
                    std::string trashIndent;
                    params[0] = "";
                    if(!vars.add_str_from_var(vpos, params[0], 0, indent, trashIndent))
                    {
                        if(!consoleLocked)
                            os_mtx->lock();
                        start_err_ml(eos, readPath, sLineNo, lineNo) << "size(): cannot get size for type " << vpos.type << std::endl;
                        os_mtx->unlock();
                        return 1;
                    }
                }
        ...
        else if(funcName == "substr")
        {
            ...
            if(replaceVars)
            {
                ...
                {
                    if(vpos.type == "string")
                        params[0] = vars.layers[vpos.layer].strings[params[0]];
                    else if(vpos.type == "int")
                        params[0] = std::to_string(vars.layers[vpos.layer].ints[params[0]]);
                    else if(vpos.type == "long long int")
                        params[0] = std::to_string(vars.layers[vpos.layer].llints[params[0]]);
                    else if(vpos.type == "double")
                        params[0] = vars.double_to_string(vars.layers[vpos.layer].doubles[params[0]], 0);
                    else
                    {
                        if(!consoleLocked)
                            os_mtx->lock();
                        start_err_ml(eos, readPath, sLineNo, lineNo) << "substr(): cannot take substring of first parameter " << params[0] << " of type " << vpos.type << std::endl;
                        os_mtx->unlock();
                        return 1;
                    }
                }
        ...
        if(funcName == "quote")
        {
            ...
            if(replaceVars)
            {
                ...
                if(vars.find(params[0], vpos))
                {
                    std::string trashIndent;
                    params[0] = "";
                    if(!vars.add_str_from_var(vpos, params[0], 1, indent, trashIndent))
                    {
                        if(!consoleLocked)
                            os_mtx->lock();
                        start_err_ml(eos, readPath, sLineNo, lineNo) << "quote(): can't get string of var type " << vpos.type << std::endl;
                        os_mtx->unlock();
                        return 1;
                    }
                }
            ...
            if(replaceVars)
            {
                ...
                if(vars.find(params[0], vpos))
                {
                    ...
                    if(!vars.add_str_from_var(vpos, params[0], 0, indent, trashIndent))
                    {
                        if(!consoleLocked)
                            os_mtx->lock();
                        start_err_ml(eos, readPath, sLineNo, lineNo) << "unquote(): can't get string of var type " << vpos.type << params.size() << std::endl;
                        os_mtx->unlock();
                        return 1;
                    }
                }
    ...
    else if(funcName[0] == 'j')
    {
        ...
        else if(funcName == "jsinclude")
        {
            ...
            if(!file_exists(jsPathStr.c_str()))
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_warn(eos, readPath, lineNo) << "jsinclude(): js file " << jsPathStr << " does not exist" << std::endl;
                if(!consoleLocked)
                    os_mtx->unlock();
            }
    ...
    {
        if(funcName == "valid-type")
        {
            if(params.size() == 0)
            {
                if(!consoleLocked)
                    os_mtx->lock();
                start_err_ml(eos, readPath, sLineNo, lineNo) << "valid-type(): expected parameters, got " << params.size() << std::endl;
                os_mtx->unlock();
                return 1;
            }
            ...
            outStr += std::to_string(valid_type(params[0], readPath, antiDepsOfReadPath, lineNo, "valid-type()", sLineNo, eos));
            if(indent)
                indentAmount += " ";
                  ...
                  std::ostream& eos)
{
    ...
    if(lang == 'n')
    {
        if(n_read_and_process_fast(1, 1, iStr, lineNo-1, readPath, antiDepsOfReadPath, str, eos))
        {
            if(!consoleLocked)
                os_mtx->lock();
            start_err(eos, readPath, callLineNo) << callType << ": failed to parse " << strType << ":" << std::endl;
            eos << iStr << std::endl;
            os_mtx->unlock();
            return 1;
        }
                          ...
                          std::ostream& eos)
{
    ...
    while(linePos < inStr.size() && inStr[linePos] != '\n')
    {
        ...
        else
        {
            ...
            for(; linePos < inStr.size() && inStr[linePos] != ' ' && inStr[linePos] != ';' && inStr[linePos] != '\n';)
            {
                if(inStr[linePos] == '\\' && linePos+1 < inStr.size())
                {
                    if(inStr[linePos+1] == 'n')
...
void Parser::print_exprtk_parser_errs(std::ostream& eos, const exprtk::parser<double>& parser, const std::string& expr_str, const Path& readPath, const int& lineNo)
{
    ...
    for(size_t i=0; i < parser.error_count(); ++i)
    {
        ...
        if(readPath.str() != "")
            eos << readPath;
        eos << "[" << c_gold << errLineNo << c_white << "]: exprtk: " << std::flush;;
